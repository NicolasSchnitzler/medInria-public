...
//----------------------------------------------------------------------------
vtkImageView3D::vtkImageView3D(): vtkImageView()
{
...
void vtkImageView3D::SetupVolumeRendering()
{
    ...
    std::cerr<<"void vtkImageView3D::SetupVolumeRendering() start"<<std::endl;
    ...
    std::cerr<<"void vtkImageView3D::SetupVolumeRendering() end"<<std::endl;
}
...
{
    std::cerr<<"void vtkImageView3D::SetInput(vtkImageData* image, vtkMatrix4x4 *matrix, int layer) start "<<image<<std::endl;
    if(layer==0)
    {
        if (this->GetInput())
        {
    ...
    std::cerr<<"void vtkImageView3D::SetInput(vtkImageData* image, vtkMatrix4x4 *matrix, int layer) before image test"<<std::endl;

    if( !image )
    {
        return;
    }
    std::cerr<<"void vtkImageView3D::SetInput(vtkImageData* image, vtkMatrix4x4 *matrix, int layer) after image test"<<std::endl;

    // Get whole extent : More reliable than dimensions if not up-to-date.
    int * w_extent = image->GetExtent();

    for(int i=0;i<6;i++)
        std::cerr<<"void vtkImageView3D::SetInput(vtkImageData* image, vtkMatrix4x4 *matrix, int layer) extent["<<i<<"]="<<w_extent[i]<<std::endl;


    int size [3] = { w_extent [1] - w_extent[0], w_extent [3] - w_extent[2], w_extent [5] - w_extent[4] };
    std::cerr<<"void vtkImageView3D::SetInput(vtkImageData* image, vtkMatrix4x4 *matrix, int layer) after extent"<<std::endl;
    ...
    std::cerr<<"void vtkImageView3D::SetInput(vtkImageData* image, vtkMatrix4x4 *matrix, int layer) "<<__LINE__<<std::endl;
    ...
    if (layer>0 && layer<4)
    {
        ...
        vtkImageData *reslicedImage = this->ResliceImageToInput(image, matrix,alg); //vtkImageData::New();
        if (!reslicedImage)
        {
        ...
        if (reslicedImage->GetScalarType()!=this->GetInput()->GetScalarType())
        {
            ...
            cast->SetInputData (reslicedImage);
    ...
    this->InternalUpdate();
    std::cerr<<"void vtkImageView3D::SetInput(vtkImageData* image, vtkMatrix4x4 *matrix, int layer) end"<<std::endl;
...
{
    std::cerr<<"void vtkImageView3D::InternalUpdate() start"<<std::endl;

    vtkSmartPointer<vtkImageData> input = this->GetInput();
    if(input!=NULL)
        std::cerr<<"void vtkImageView3D::InternalUpdate() after GetInput: "<<*input<<std::endl;
    ...
    std::cerr<<"void vtkImageView3D::InternalUpdate() before setInputData: "<< appender->GetOutputPort()<<std::endl;

    this->VolumeMapper->SetInputConnection(    appender->GetOutputPort());
    std::cerr<<"void vtkImageView3D::InternalUpdate() before setInputData: "<< appender->GetOutput()->HasScalarType(appender->GetOutputInformation(0))<<std::endl;
    this->VolumeMapper->Update();
    ...
    std::cerr<<"void vtkImageView3D::InternalUpdate() endl"<<std::endl;
}
...
vtkActor* vtkImageView3D::AddDataSet (vtkPointSet* arg, vtkProperty* prop)
{
    ...
    normalextractor->SetInputData (geometryextractor->GetOutput());
    mapper->SetInputData (normalextractor->GetOutput());
